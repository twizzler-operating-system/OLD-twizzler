...
void encap_arp_packet(uint16_t opcode, mac_addr_t dst_mac, ip_addr_t dst_ip)
{
	...
	/* add Ethernet header */
	mac_addr_t broadcast_mac = (mac_addr_t){ .mac = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF } };
	eth_tx(
	  interface_name, (opcode == ARP_REQUEST) ? broadcast_mac : dst_mac, ARP, pkt_ptr, pkt_size);
}
  ...
  uint16_t payload_size)
{
	...
	if(has_twz_hdr) {
		pkt_size = ETH_HDR_SIZE + TWZ_HDR_SIZE + IP_HDR_SIZE + UDP_HDR_SIZE + payload_size;
	} else {
	...
	ip_tx(interface_name, dst_ip, UDP, ip_ptr, (IP_HDR_SIZE + UDP_HDR_SIZE + payload_size));

	if(has_twz_hdr) {
		/* add Twizzler header */
		char *twz_ptr = (char *)pkt_ptr;
		twz_ptr += (pkt_size - TWZ_HDR_SIZE - IP_HDR_SIZE - UDP_HDR_SIZE - payload_size);
  ...
  uint16_t payload_size)
{
...
#else
	pkt_size = ETH_HDR_SIZE + IP_HDR_SIZE + TCP_HDR_SIZE + (has_twz_hdr ? TWZ_HDR_SIZE : 0);
#endif
	if(pkt_size > MAX_ETH_FRAME_SIZE) {
		return EMAXFRAMESIZE;
	}
	...
	if(has_twz_hdr) {
		...
		char *twz_ptr = (char *)pkt_ptr;
#if 0
		twz_ptr += (pkt_size - TWZ_HDR_SIZE - IP_HDR_SIZE - TCP_HDR_SIZE - payload_size);
#else
		twz_ptr += (pkt_size - TWZ_HDR_SIZE - IP_HDR_SIZE - TCP_HDR_SIZE);
#endif
		twz_tx(object_id, twz_op, IPV4, twz_ptr);
	}
