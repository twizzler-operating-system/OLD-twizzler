...
/* creates a char ring buffer with capacity 'size' */
char_ring_buffer_t* create_char_ring_buffer(uint32_t size);

/* copies bytes amount of data from src_buf to the tail of ring buffer
...
/* removes a max of bytes amount of data from the head of ring buffer
 * and adds it to dst_buff
...
/* copies a max of bytes amount of data from index *idx of ring buffer to dst_buff
