...
$(MUSL_HDRS): $(BUILDDIR)/us/musl-config.mk $(MUSL_SRCS)
	...
	-@cd $(BUILDDIR)/us/sysroot/usr/include && [ ! -e twz ] && ln -s ../../../../../../../us/include/twz nstack
	@touch $@
...
TWZCFLAGS=-Wall -Wextra -O3 -g -march=native -mclflushopt -mno-avx512f -mno-avx2 #-mclwb
...
INITRD_FILES=usr/bin/init usr/bin/init_bootstrap usr/bin/init_bootstrap.data usr/bin/logboi usr/lib/libtwz.so usr/lib/libtwix.so usr/lib/libc.so lib/ld64.so lib/ld64.so.1 usr/lib/libgcc_s.so usr/lib/libgcc_s.so.1 usr/lib/libstdc++.so usr/lib/libstdc++.so.6 usr/lib/libstdc++.so.6.0.27 usr/bin/pager usr/bin/nvme usr/bin/keyboard usr/bin/serial usr/bin/twzdev usr/lib/libbacktrace.so usr/lib/libbacktrace.so.0 usr/lib/libbacktrace.so.0.0.0
