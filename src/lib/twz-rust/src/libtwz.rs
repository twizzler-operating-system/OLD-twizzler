...
impl QueueMultiSpec {
	pub(crate) fn new_subm<S: Copy, C: Copy>(
		queue: &crate::queue::Queue<S, C>,
		result: &mut crate::queue::QueueEntry<S>,
	) -> QueueMultiSpec {
		queue.obj().alloc_libtwz_data();
		QueueMultiSpec {
			obj_or_word: ObjOrWord {
				obj: queue.obj().libtwz_data.lock().unwrap().as_mut().unwrap().data as *mut std::ffi::c_void,
			},
			result: unsafe {
				std::mem::transmute::<*mut crate::queue::QueueEntry<S>, *mut std::ffi::c_void>(
					result as *mut crate::queue::QueueEntry<S>,
				)
			},
			cmp: 0,
			sq: 0,
	...
	pub(crate) fn new_sleep(word: &std::sync::atomic::AtomicU64, cmp: u64) -> QueueMultiSpec {
		QueueMultiSpec {
			obj_or_word: ObjOrWord {
				word: word as *const std::sync::atomic::AtomicU64,
			},
	...
	pub(crate) fn new_cmpl<S: Copy, C: Copy>(
		queue: &crate::queue::Queue<S, C>,
		result: &mut crate::queue::QueueEntry<C>,
	) -> QueueMultiSpec {
		queue.obj().alloc_libtwz_data();
		QueueMultiSpec {
			obj_or_word: ObjOrWord {
			...
			result: unsafe {
				std::mem::transmute::<*mut crate::queue::QueueEntry<C>, *mut std::ffi::c_void>(
					result as *mut crate::queue::QueueEntry<C>,
				)
			},
			cmp: 0,
			sq: 1,
...
pub(crate) mod twz_c {
	...
	#[allow(dead_code)]
	extern "C" {
		pub(crate) fn twz_name_resolve(
			_obj: *mut std::ffi::c_void,
			name: *const i8,
			_fn: *const std::ffi::c_void,
			flags: i32,
			id: *mut LibtwzObjID,
		) -> i32;
		...
		pub(crate) fn twz_object_init_ptr(data: *mut std::ffi::c_void, p: *const i8);
		pub(crate) fn twz_view_allocate_slot(_obj: *mut std::ffi::c_void, id: LibtwzObjID, flags: u32) -> i64;
		pub(crate) fn twz_alloc(
			obj: *mut std::ffi::c_void,
			len: usize,
			owner: *mut *const std::ffi::c_void,
			flags: u64,
			ctor: extern "C" fn(*mut std::ffi::c_void, *mut std::ffi::c_void),
			data: *mut std::ffi::c_void,
		) -> i32;

		pub(crate) fn twz_free(
			obj: *mut std::ffi::c_void,
		...
		pub(crate) fn twz_fault_set(
			fault: i32,
			cb: Option<extern "C" fn(i32, *mut std::ffi::c_void, *mut std::ffi::c_void)>,
			data: *mut std::ffi::c_void,
		) -> i32;
		pub(crate) fn twz_fault_set_upcall_entry(cb: Option<extern "C" fn()>, dbl_cb: Option<extern "C" fn()>);
		...
		pub(crate) fn bstream_write(obj: *mut std::ffi::c_void, ptr: *const u8, len: usize, flags: u32) -> i64;
		pub(crate) fn bstream_read(obj: *mut std::ffi::c_void, ptr: *mut u8, len: usize, flags: u32) -> i64;
		pub(crate) fn bstream_obj_init(obj: *mut std::ffi::c_void, hdr: *mut std::ffi::c_void, nbits: u32) -> i32;
		pub(crate) fn __twz_ptr_store_guid(
			obj: *mut std::ffi::c_void,
			ptr: *mut u64,
			dest_obj: *mut std::ffi::c_void,
			p: u64,
			f: u64,
		) -> i32;
		...
		pub(crate) fn twz_object_kaction(obj: *mut std::ffi::c_void, cmd: u64, arg: u64) -> i32;
	}
}
...
pub(crate) fn twz_object_guid(obj: &Twzobj) -> ObjID {
	unsafe {
		...
		twz_c::twz_object_guid(obj.libtwz_data.lock().unwrap().as_mut().unwrap().data as *mut std::ffi::c_void)
	}
}

pub(crate) fn twz_object_kaction(obj: &Twzobj, cmd: u64, arg: u64) -> i32 {
	unsafe {
		obj.alloc_libtwz_data();
		twz_c::twz_object_kaction(
...
pub(crate) fn queue_submit<T>(obj: &Twzobj, item: &T, flags: i32) -> i32 {
	unsafe {
		...
		twz_c::queue_submit(
			obj.libtwz_data.lock().unwrap().as_mut().unwrap().data as *mut std::ffi::c_void,
			std::mem::transmute::<&T, *const std::ffi::c_void>(item),
			flags,
		)
	}
}
...
pub(crate) fn queue_receive<T>(obj: &Twzobj, item: &mut T, flags: i32) -> i32 {
	unsafe {
		...
		twz_c::queue_receive(
			obj.libtwz_data.lock().unwrap().as_mut().unwrap().data as *mut std::ffi::c_void,
			std::mem::transmute::<&mut T, *mut std::ffi::c_void>(item),
			flags,
		)
	}
}
...
pub(crate) fn queue_multi_wait(count: usize, specs: &mut [QueueMultiSpec]) -> i64 {
	unsafe {
		twz_c::queue_dequeue_multiple(
			count,
			std::mem::transmute::<*mut QueueMultiSpec, *mut std::ffi::c_void>(specs.as_mut_ptr()),
		)
	}
}
...
pub(crate) fn twz_object_alloc_move<T>(
	obj: &Twzobj,
	owner: &mut crate::ptr::Pptr<T>,
	flags: u64,
	ctor: extern "C" fn(&mut T, &T),
	item: T,
) -> i32 {
	unsafe {
		obj.alloc_libtwz_data();
		twz_c::twz_alloc(
			...
			flags,
			std::mem::transmute::<extern "C" fn(&mut T, &T), extern "C" fn(*mut std::ffi::c_void, *mut std::ffi::c_void)>(
				ctor,
			),
...
pub(crate) fn twz_object_alloc_slice_move<T: Copy>(obj: &Twzobj, owner: &mut u64, flags: u64, slice: &[T]) -> i32 {
	...
	extern "C" fn ctor<F: Copy>(ptr: *mut F, copy: &CopyData<F>) {
		for i in 0..copy.len {
			unsafe {
...
pub(crate) fn twz_object_create(flags: i32, kuid: ObjID, src: ObjID) -> Result<ObjID, i32> {
	...
	let res = unsafe { twz_c::twz_object_create(flags, kuid, src, &mut oid) };
	if res < 0 {
		Err(res)
	} else {
		Ok(oid)
