...
impl<'a, T, R> Pref<'a, T, R> {
	pub(super) fn new(obj: Option<&'a Twzobj<T>>, p: &'a R) -> Pref<'a, T, R> {
		Pref { obj, p }
	}
}
...
impl<R> Pptr<R> {
	...
	pub fn lea<'a, T>(&'a self) -> Pref<'a, T, R> {
		if self.is_internal() {
			return Pref {
				obj: None,
				p: unsafe {
	...
	pub fn lea_obj<'a, T>(&self, obj: &Twzobj<T>) -> Pref<'a, T, R> {
		if self.is_internal() {
			Pref {
				obj: None,
