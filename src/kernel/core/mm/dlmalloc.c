...
/* this is dlmalloc ported to twizzler kernel. Here are some wrapper functions. */
#include <memory.h>

static inline void *sbrk(intptr_t x)
{
	/* we only support mmap-style allocations */
	(void)x;
...
static inline int munmap(void *addr, size_t sz)
{
	(void)sz;
...
{
	spinlock_acquire_save(&dlm_lock);
	void *r = realloc(p, sz);
	spinlock_release_restore(&dlm_lock);
	// printk("krealloc: %p %lx -> %p\n", p, sz, r);
	return r;
}

void *krecalloc(void *p, size_t num, size_t sz)
{
	void *r = krealloc(p, num * sz);
	return r;
}
...
#define MALLOC_FAILURE_ACTION panic("malloc failure")
...
#define malloc_getpagesize mm_page_size(0)
...
#define MMAP(addr, size, prot, flags) (twz_mmap((addr), (size), (prot), (flags)))
