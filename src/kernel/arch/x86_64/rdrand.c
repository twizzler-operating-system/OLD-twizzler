...
static ssize_t __get(void *p, size_t l)
{
	return rdrand_get_bytes(l, p);
}

static struct entropy_source source = {
	.name = "rdrand",
	.get = __get,
};

__initializer static void __init_rdrand(void)
{
	rand_register_entropy_source(&source);
}
