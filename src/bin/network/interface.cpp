  ...
  ip_addr_t interface_bcast_ip)
{
	...
	if(twz_object_new(
	     &interface_obj, NULL, NULL, TWZ_OC_DFL_READ | TWZ_OC_DFL_WRITE | TWZ_OC_TIED_NONE)
	   < 0) {
		fprintf(stderr,
		  "Error init_interface: "
	...
	while(!(flags & NIC_FL_MAC_VALID)) {
		...
		(void)twz_thread_sync(THREAD_SYNC_SLEEP, &nh->flags, flags, /* timeout */ NULL);
	...
	if(interface_list.insert(std::make_pair(interface_name, interface)).second == false) {
	...
	if(twz_object_init_name(&interface->tx_queue_obj, tx_queue_name, FE_READ | FE_WRITE) < 0) {
	...
	if(twz_object_init_name(&interface->rx_queue_obj, rx_queue_name, FE_READ | FE_WRITE) < 0) {
...
void get_interface_by_ip(ip_addr_t ip, char *interface_name)
{
	std::map<const char *, interface_t *>::iterator it;
...
int bind_to_ip(ip_addr_t ip)
{
	...
	if(!compare_ip_addr(ip, default_ip, default_ip)) {
		...
		fprintf(stderr,
		  "Error bind_to_ip: ip address %d.%d.%d.%d does not exist\n",
		  ip.ip[0],
		  ip.ip[1],
		  ip.ip[2],
		  ip.ip[3]);
