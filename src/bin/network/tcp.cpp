  ...
  uint16_t payloadsz)
{
	...
	uint8_t flags =
	  cwr << 7 | ece << 6 | urg << 5 | ack << 4 | psh << 3 | rst << 2 | syn << 1 | fin;
	...
	pseudo_ip_hdr.len = htons(tcp_pkt_size + payloadsz);
	...
	// memcpy(hdr + PSEUDO_IP_HDR_SIZE, payload,
	// memcpy(hdr + PSEUDO_IP_HDR_SIZE, tcp, tcp_pkt_size);

	// fprintf(stderr, "SEND %d %d %x\n", seq_num, ack_num, window_size);
	// fprintf(stderr, "===> %d %d %d\n", PSEUDO_IP_HDR_SIZE, tcp_pkt_size, payloadsz);
	uint32_t cs32 = partial_checksum(0, (unsigned char *)pseudo_ip, PSEUDO_IP_HDR_SIZE, false);
	cs32 = partial_checksum(cs32, (unsigned char *)tcp, tcp_pkt_size, false);
	cs32 = partial_checksum(cs32, (unsigned char *)payload, payloadsz, true);
	uint16_t cs = finish_partial_checksum(cs32);
	// fprintf(stderr, "    => %x\n", cs);
	tcp_hdr->checksum = cs;
...
void tcp_rx(const char *interface_name, remote_info_t *remote_info, void *tcp_pkt_ptr)
{
	...
	uint16_t recvd_checksum = tcp_hdr->checksum;
	tcp_hdr->checksum = 0;
...
*/
	uint32_t sum = partial_checksum(0, (unsigned char *)pseudo_ip, PSEUDO_IP_HDR_SIZE, false);
	sum = partial_checksum(sum, (unsigned char *)tcp, remote_info->ip_payload_size, true);
	uint16_t calculated_checksum = finish_partial_checksum(sum);
	if(recvd_checksum != calculated_checksum) {
		fprintf(stderr,
		  "tcp_rx: checksum mismatch (%x %x); packet dropped\n",
		  recvd_checksum,
		  calculated_checksum);
		return;
	}
	...
	handle_tcp_recv(interface_name,
	  remote_info,
	  dst_port,
	  payload,
